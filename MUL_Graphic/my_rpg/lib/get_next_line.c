...
static char *strconcat(char *dest, char *src)
{
...
static int get_next_line3(char ***str, int a, char **result, char (*mem)[100+1])
{
...
static char *get_next_line2(char (*mem)[100+1], char **result, int fd)
{
...
{
    char *result = NULL;
    static char mem[100+1];

    if (fd < 0 || 100 < 1)
        return (NULL);
    for (int i = 0; mem != NULL && mem[i] != '\0'; i++)
        if (mem[i] == '\n') {
